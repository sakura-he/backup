...
setupMock({
	setup: () => {
		...
		mock(new RegExp("api/monitor/sale"), (options) => {
			type chartDataType = { date: string, value: number }
			enum dateTypeEnum {
				month = 3,
				week = 2,
				day = 1
			...
			let reqDateType: dateTypeEnum =
				Number.parseInt(url.searchParams.get('type') || dateTypeEnum['day'].toString())
			switch (reqDateType) {
				case dateTypeEnum['day']:
					for (let index = 0; index <= dayjs().hour(); index++) {
						...
						if (index === dayjs().hour()) {
							...
							data.date = dayjs().hour(index).minute(59).format('HH:59');
				...
				case dateTypeEnum['week']:
					for (let index = 1; index <= 7; index++) {
						...
						chartData.push({
							date: dayjs().dayOfYear(dayjs().dayOfYear() - currentDay).format('MM-DD'),
